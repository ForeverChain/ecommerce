...
router.put("/:id", verifyTokenAndAuthorization, async (req, res) => {
    ...
    try {
        ...
        if (updatedUser == null) {
            ...
            res.status(200).json({ success: 1, message: "Хэрэглэгчийн мэдээлэл амжилттай шинчлэгдлээ", data: [updatedUser] });
        }
    } catch (err) {
        res.status(500).json({ status: 0, message: err.message });
    }
...
router.delete("/:id", verifyTokenAndAuthorization, async (req, res) => {
    try {
        ...
        if (deletedUser == null) {
            ...
            res.status(200).json({ success: 1, message: "Хэрэглэгч амжилттай устгагдлаа" });
        }
    } catch (err) {
        res.status(500).json({ status: 0, message: err.message });
    }
