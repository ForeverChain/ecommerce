...
const CreateProduct = () => {
    ...
    const handleSubmit = (e) => {
        ...
        const { title, description, address, image, categories, color, price, stock } = formState.values;
        const seller = userInfo?.data[0]?._id;

        if (title && description && address && image && categories && color && price && stock && seller) {
            dispatch(addProducts({ title, description, address, image, categories, color, price, stock, seller }));
        }
    };
                        ...
                        <div className="user_card content-card">
                            ...
                            <div className="justify-content-center form_container auth-page-container">
                                <form onSubmit={handleSubmit} autoComplete="off">
                                    ...
                                    {submitted && !formState.values.address && <div className="inline-errormsg">Address is required</div>}
                                    <div className="input-group mt-3">
