...
const CreateShop = () => {
    ...
    const handleSubmit = (e) => {
        ...
        const { name, description, address, image, facebook, instagram, mail } = formState.values;
        const seller = userInfo?.data[0]?._id;
        if ((name && description && address, image, mail && seller)) {
            dispatch(addShop({ name, description, address, image, facebook, instagram, mail, seller }));
        }
    };
